...
export default class BlogList extends Component {
  scrollTop() {
    $('html, body').animate({
      scrolltTop: $('#main-content').offset().top
    }, 500)
  }
  render() {
    ...
    let articles_html = articles.map((article) => {
      let data_obj = new Date(article.created)
      ...
      return(
        <div key={'key-' + article.slug}>
          <div className='post-preview'>
            ...
            <p className='post-meta'>Post by <a href='https://cosmicjs.com' target='_blank'>Cosmic JS</a> on {created}</p>
          </div>
          <hr/>
        </div>
      )
